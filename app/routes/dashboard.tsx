    ...
    >
      ...
      >
        ...
        <motion.div
          className="absolute rounded-full"
          style={{
            width: miniPlanetSize * 1.7,
            height: miniPlanetSize * 1.7,
            left: '50%',
            top: '50%',
            marginLeft: -(miniPlanetSize * 0.85) / 2,
            marginTop: -(miniPlanetSize * 0.85) / 2,
            backgroundColor: 'var(--green)',
            boxShadow: '0 0 18px #82E2C0, inset -3px -3px 6px rgba(0, 0, 0, 0.4)',
          }}
          animate={{
            x: Array.from({ length: 61 }, (_, i) => {
...
function Starfield() {
  ...
  useEffect(() => {
    ...
    const stars = Array.from({ length: 140 }, () => ({
      ...
      r: Math.random() * 0.9 + 0.2, a: Math.random(),
    ...
    const draw = () => {
      ...
      stars.forEach(s => {
        ...
        ctx.fillStyle = `rgba(240,240,240,${s.a * 0.5})`; ctx.fill();
      });
  ...
  return (
    <div id="orbit-root" style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <nav className={`orbit-nav transition-all duration-500 border-b border-white/10 backdrop-blur-md bg-black/40 px-8 py-4 flex items-center justify-between
          ${isScrolled ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`} style={{ justifyContent: "space-between" }}>
        ...
        {/* sticky status bar — shown when hero is hidden */}
        <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{
            ...
            background: "linear-gradient(135deg, #4AABF0 0%, #82E2C0 100%)",
      ...
      {/* ── HERO ── */}
      <section  className={`hero transition-all duration-700 ${isScrolled ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          <motion.div
        ...
        <h1 className="hero-title">Hello, <em>JobNaut.</em></h1>
        <p className="hero-title-sub">Ready for orbiting new opportunities?</p>
        ...
        <div className="hero-actions">
          ...
          <button className="btn-secondary">Configure Profile</button>
        </div>

        <div className="stats-bar" style={{ width: "100%", maxWidth: 600 }}>
          {[
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", minHeight: "100vh" }}>
        ...
        {/* SIDEBAR */}
        <aside style={{
          width: 220, flexShrink: 0, position: "fixed", top: 60, height: "calc(100vh - 60px)",
          borderRight: "1px solid var(--border)", background: "var(--bg2)",
          display: "flex", flexDirection: "column", padding: "32px 0",
        }}>
          <div style={{ padding: "0 20px 24px", borderBottom: "1px solid var(--border)" }}>
            <div style={{ fontSize: 9, letterSpacing: "2px", fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", marginBottom: 8 }}>Navigation</div>
          </div>
          <nav style={{ padding: "16px 12px", flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
            {NAV_ITEMS.map(item => {
              const icons = { Dashboard: "◈", Analytics: "◎", Offers: "⌬", Settings: "◉" };
              const active = sidebarActive === item;
              return (
                <button key={item} onClick={() => setSidebarActive(item)}
                  style={{
                    display: "flex", alignItems: "center", gap: 10,
                    padding: "10px 14px", borderRadius: 6, border: "none",
                    background: active ? "rgba(74,171,240,0.08)" : "transparent",
                  ...
                  <span style={{ fontSize: 14, opacity: .8 }}>{icons[item]}</span>
                  {item}
                </button>
              );
            })}
          ...
          <div style={{ margin: "12px", padding: "14px 16px", background: "var(--bg3)", borderRadius: 8, border: "1px solid var(--border)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#82E2C0", boxShadow: "0 0 6px #82E2C0" }} />
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "2px", color: "#82E2C0", fontFamily: "'DM Mono',monospace" }}>AI SCANNING</span>
            </div>
            <div style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.5 }}>Last sync <span style={{ color: "var(--white)", fontFamily: "'DM Mono',monospace" }}>2m ago</span></div>
            <div style={{ fontSize: 11, color: "var(--muted)" }}>Found <span style={{ color: "var(--cyan)", fontFamily: "'DM Mono',monospace" }}>+47</span> new today</div>
          </div>
        </aside>

        {/* MAIN CONTENT */}
        <main style={{ flex: 1, padding: "48px 48px 80px", maxWidth: "calc(100% - 220px)" }}>
          ...
          <footer style={{
            ...
            <div className="footer-copy">© 2025 Orbit · All systems nominal</div>
          </footer>
